document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('searchButton').addEventListener('click', async () => {
        ...
        try {
            ...
            const racks = roomConfigData.data.racks; // Supondo que racks seja parte do roomConfigData.data

            // Associar cada minerador à sua rack
            minerData.forEach(miner => {
                const rack = racks.find(r => r.rack_id === miner.rack_id); // Encontrar a rack correspondente
                if (rack) {
                    miner.rack_info = rack; // Adiciona as informações da rack ao miner
                }
            });
            ...
            const updateElement = (index, miner) => {
                if (miner) {
                    ...
                    // Exibir informações da rack
                    if (miner.rack_info) {
                        const rack = miner.rack_info;
