...
<body>
    ...
    <script>
        ...
        async function fetchUserDataWithRetry(userId, retries = 3, delay = 1000) {
            ...
            throw new Error(`Erro ao buscar dados para o usuário ID ${userId} após ${retries} tentativas.`);
        }
        ...
        async function fetchAndDisplayAllUsers() {
            ...
            for (const user of initialPowerData) {
                try {
                    ...
                    if (!userData) {
                        console.error(`Erro ao buscar dados para o usuário ID ${user.id}`);
                        break;  // Interrompe a execução ao primeiro erro após 3 tentativas
                    }

                    const userTotalPower = userData.miners + (userData.miners * userData.bonus_percent / 100) + userData.racks;
                    let positionChange = 0;

                    if (previousPower !== null) {
                        const positionDifference = previousPower.totalPower - userTotalPower;
                        if (positionDifference !== 0) {
                            positionChange = Math.sign(positionDifference);
                        }
                    }

                    previousPower = { totalPower: userTotalPower, rank };

                    addDataToTable(user, userData, user.inicial, rank, positionChange);

                    rank++;
                    currentProgress++;
                    const progressPercentage = (currentProgress / totalUsers) * 100;
